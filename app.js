...
app.post("/login", (req, res) => {
  ...
  fs.readFile("users.json", "utf8", (err, data) => {
    ...
    if (user) {
      ...
      req.session.user = { username: user.username }; // Store the user in session
...
app.get("/quizzes", isAuthenticated, (req, res) => {
  ...
  res.send(`
        ...
        <ul>
            ${quizFiles
              .map((file) => `<li><a href="/quiz/${file}">${file}</a></li>`)
              .join("")}
        </ul>
    `);
});
    ...
    if (questionMatch) {
      ...
      if (correctAnswerMatch) {
        currentQuestion.correctAnswers.push(
          correctAnswerMatch[1].toLowerCase().trim()
        ); // Store correct answer for text input
      }
...
app.post("/submit", isAuthenticated, preventBackNavigation, (req, res) => {
  ...
  parsedQuestions.forEach((question, index) => {
    ...
    if (question.type === "multiple") {
      const userAnswersNormalized = userAnswerArray
        .map((a) => a.trim().toLowerCase())
        .sort();
      const correctAnswersNormalized = question.correctAnswers
        .map((a) => a.trim().toLowerCase())
        .sort();
      isCorrect =
        userAnswersNormalized.toString() ===
        correctAnswersNormalized.toString();
    } else if (question.type === "single") {
      isCorrect =
        userAnswerArray[0]?.trim().toLowerCase() ===
        question.correctAnswers[0]?.toLowerCase();
    } else if (question.type === "text") {
      const userTextAnswer = userAnswerArray[0]?.trim().toLowerCase();
      const correctTextAnswer = question.correctAnswers[0]
        ?.trim()
        .toLowerCase();
      isCorrect =
        userTextAnswer &&
        correctTextAnswer &&
        userTextAnswer === correctTextAnswer;
    }
    ...
    results.push({
      ...
      userAnswer: userAnswerArray.length
        ? userAnswerArray
        : ["No answer provided"],
      correct: isCorrect,
  ...
  req.session.previousScores.push({
    score: scorePercentage,
    totalCorrect: totalCorrect,
    totalQuestions: totalQuestions,
    date: new Date().toLocaleString(),
  }); // Store score and date
                    ...
                    <li class="${result.correct ? "correct" : "wrong"}">
                      ...
                      result.questionText
                        ...
                        ${
                          result.correct
                            ? ""
                            : `Correct Answer: ${result.correctAnswers.join(
                                ", "
                              )}`
                        }
                    </li>
...
app.get("/results", isAuthenticated, (req, res) => {
  ...
  res.send(`
        ...
        <div class="container">
            ...
            <h2>Your Score: ${previousScores.map(
              (score) =>
                `${score.score}% (${totalCorrect} out of ${totalQuestions})`
            )}</h2>
            <a href="/quizzes">Back to Quizzes</a>
        </div>
    `);
